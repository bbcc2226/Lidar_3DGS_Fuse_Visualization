#!/usr/bin/env python3
"""Back-project depth maps and voxel-fuse them into a colored point cloud."""
...
def main():
    ...
    p.add_argument(
        "--poses",
        type=Path,
        default=Path("output/lio_camera_pose_incremental/poses_optimized_tum.txt"),
    )
    p.add_argument(
        "--output",
        type=Path,
        default=Path("output/lio_camera_pose_incremental/dense_fused.ply"),
    )
    p.add_argument(
        "--text-output",
        type=Path,
        help="Optional x y z r g b frame_support text export",
    )
    ...
    a = p.parse_args()
    K = np.loadtxt(a.data / "intrinsics.txt", delimiter=",")
    ...
    for index, (name, stamp) in enumerate(lines):
        ...
        if image is None or depth is None:
            continue
        ys = np.arange(0, depth.shape[0], a.stride)
        xs = np.arange(0, depth.shape[1], a.stride)
        u, v = np.meshgrid(xs, ys)
        d = depth[v, u]
        valid = np.isfinite(d) & (d > 0.1) & (d < a.max_depth)
        ...
        xyz_sum = np.zeros((len(uniq), 3))
        rgb_sum = np.zeros((len(uniq), 3))
        cnt = np.zeros(len(uniq), np.int64)
        np.add.at(xyz_sum, inv, pw)
        np.add.at(rgb_sum, inv, rgb)
        np.add.at(cnt, inv, 1)
        xyz = xyz_sum / cnt[:, None]
        col = rgb_sum / cnt[:, None]
        for q, x, c, n in zip(map(tuple, uniq), xyz, col, cnt):
            if q in acc:
                ...
                acc[q] = [x.copy(), c.copy(), 1, int(n)]
        raw += len(d)
        used_frames += 1
        if (index + 1) % 50 == 0:
            print("frames", index + 1, "raw", raw, "voxels", len(acc), flush=True)
    ...
    metrics = {
        ...
        "images_used": used_frames,
        "raw_depth_samples": raw,
        "voxel_size_m": a.voxel,
        "min_frame_support": a.min_frame_support,
