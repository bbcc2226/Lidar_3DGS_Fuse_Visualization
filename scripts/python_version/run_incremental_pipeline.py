...
def main():
    ...
    p.add_argument('--output',type=Path,default=Path('output/lio_camera_pose_incremental'))
    ...
    p.add_argument('--loop-max-distance',type=float,default=.5)
    p.add_argument('--loop-max-per-frame',type=int,default=3)
    p.add_argument('--resume',action='store_true')
    a=p.parse_args()
    available=sum(1 for line in open(a.data/'timestamps.txt')
                  if (a.data/'undistorted'/line.split()[0]).exists())
    total=available if a.max_images<0 else min(available,a.max_images)
    a.output.mkdir(parents=True,exist_ok=True);history_path=a.output/'incremental_history.json'
    if a.resume and history_path.exists():
        history=json.load(open(history_path))
        last_global=max((x['active_images'] for x in history if x['global_ba']),default=0)
        active=min(total,history[-1]['active_images']+a.batch_size)
        print('RESUME',history[-1]['active_images'],'->',active,'last_global',last_global,flush=True)
    else:
        run(['./build/lio_camera_pose','--data',a.data,'--output',a.output,
             '--max-images',total,'--time-offset','-0.4'])
        history=[];active=min(a.bootstrap,total);last_global=0
    while True:
        run(['python3','standalone/triangulate_sparse.py','--data',a.data,
             '--output',a.output,'--max-images',active,
             '--max-pair-gap',a.recent_window,
             '--loop-min-separation',a.loop_min_separation,
             '--loop-max-distance',a.loop_max_distance,
             '--loop-max-per-frame',a.loop_max_per_frame,
             '--min-points','1'])
        ...
        cmd=['./build/lio_bundle_adjust','--data',a.data,'--output',a.output,
             '--max-images',active,'--local-start',max(0,active-15)]
        if not do_global:cmd+=['--no-global']
        run(cmd,allow_two=True)
        ev=json.load(open(a.output/'evaluation.json'))
        history.append(dict(active_images=active,global_ba=do_global,
            landmarks=tri['triangulated_points'],observations=tri['observations'],
            loop_candidates=tri.get('loop_candidates',0),
            accepted_loop_matches=tri.get('accepted_loop_matches',0),
            conflicting_track_merges_rejected=tri.get('conflicting_track_merges_rejected',0),
        ...
        if do_global:last_global=active
