...
def colorize(depth, valid, max_vis_depth):
    ...
    color=cv2.applyColorMap(np.rint(255*scaled).astype(np.uint8),cv2.COLORMAP_TURBO)
    color[~valid]=0
    return color
...
def label(im,text):
    ...
    cv2.putText(out,text,(12,27),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,255,255),2,cv2.LINE_AA)
    return out
...
def main():
    ...
    p.add_argument('--min-depth',type=float,default=.1)
    p.add_argument('--max-depth',type=float,default=30.)
    p.add_argument('--sample-frames',type=int,nargs='*',default=[50,108,150,250,400,520])
    a=p.parse_args()
    for d in ('depth_npy','depth_mm','masks','confidence','previews'):
        (a.output/d).mkdir(parents=True,exist_ok=True)
    entries=[]; total_valid=0; total_pixels=0; previews=[]
    ...
    for frame,(name,stamp) in enumerate(stamps):
        ...
        if depth.ndim!=2: raise ValueError(f'expected scalar depth: {source}')
        if depth.shape!=image.shape[:2]:
            raise ValueError(f'depth/image size mismatch: {name} {depth.shape} {image.shape[:2]}')
        ...
        cv2.imwrite(str(a.output/'masks'/(stem+'.png')),valid.astype(np.uint8)*255)
        cv2.imwrite(str(a.output/'confidence'/(stem+'.png')),np.rint(conf*65535).astype(np.uint16))
        count=int(valid.sum()); total_valid+=count;total_pixels+=valid.size
        entries.append({'frame':frame,'image':name,'timestamp':float(stamp),
                        'valid_pixels':count,'coverage':count/valid.size,
        ...
        if frame in a.sample_frames:
            ...
            overlay=image.copy();overlay[valid]=np.rint(.35*image[valid]+.65*color[valid]).astype(np.uint8)
            panel=np.hstack((label(image,'Original'),label(color,'LiDAR depth'),label(overlay,'Depth overlay')))
            out=a.output/'previews'/(f'{frame:06d}_{stem}_comparison.jpg')
            cv2.imwrite(str(out),panel,[cv2.IMWRITE_JPEG_QUALITY,94]);previews.append(str(out))
    intrinsics=a.reconstruction/'intrinsics_refined.txt'
    if not intrinsics.exists(): intrinsics=a.reconstruction/'intrinsics_input.txt'
    if not intrinsics.exists(): intrinsics=a.data/'intrinsics.txt'
    report={'format':{'depth_npy':'float32 meters; invalid=0','depth_mm':'uint16 millimeters; invalid=0',
                      'mask':'uint8 0/255','confidence':'uint16 normalized local sampling density'},
            'source':'LiDAR-derived camera-frame z-depth','reconstruction':str(a.reconstruction),
            'poses':str(a.reconstruction/'poses_optimized_tum.txt'),'intrinsics':str(intrinsics),
            'frames_written':len(entries),'valid_pixels':total_valid,
            'mean_coverage':total_valid/max(total_pixels,1),'sample_previews':previews,
            'frames':entries}
    (a.output/'depth_supervision.json').write_text(json.dumps(report,indent=2)+'\n')
