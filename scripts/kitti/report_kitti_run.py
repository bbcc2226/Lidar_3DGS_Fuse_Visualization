...
def main():
    ...
    ap.add_argument(
        "--image-directory",
        type=Path,
        help="Source images for reports that also include reconstruction-only context frames",
    )
    args = ap.parse_args()
    root = args.reconstruction
    report = args.dataset / "report"
    report.mkdir(exist_ok=True)
    overlays = report / "frames"
    ...
    names = [
        s.split()[0]
        for s in (args.prepared / "timestamps.txt").read_text().splitlines()
        if s and not s.startswith("#")
    ]
    ids = np.array([int(Path(s).stem) for s in names])
    n = len(names)
    prior = np.loadtxt(root / "poses_lio_prior_tum.txt")
    final = np.loadtxt(root / "poses_optimized_tum.txt")
    assert len(prior) == len(final) == n
    assert np.allclose(prior[:, 0], final[:, 0], rtol=0, atol=1e-5)
    rp = Rotation.from_quat(prior[:, 4:8]).as_matrix()
    rf = Rotation.from_quat(final[:, 4:8]).as_matrix()
    shift = np.linalg.norm(final[:, 1:4] - prior[:, 1:4], axis=1)
    angle = (
        Rotation.from_matrix(np.einsum("nji,njk->nik", rp, rf)).magnitude()
        * 180
        / np.pi
    )
    k = np.loadtxt(args.prepared / "intrinsics.txt", delimiter=",")
    ki = np.linalg.inv(k)
    points = {}
    lengths = {}
    for line in (root / "sparse/0/points3D.txt").read_text().splitlines():
        ...
        s = line.split()
        idx = int(s[0]) - 1
        points[idx] = [float(x) for x in s[1:4]]
        lengths[idx] = (len(s) - 8) // 2
    obs = np.genfromtxt(root / "observations.csv", delimiter=",", names=True)
    ci = obs["camera_id"].astype(int)
    li = obs["landmark_id"].astype(int)
    xyz = np.array([points[i] for i in li])
    uv = np.column_stack((obs["u"], obs["v"]))
    ...
    before = errors(prior, rp)
    after = errors(final, rf)
    ...
    grids = np.zeros((n, 6, 8), dtype=int)
    rows = []
    for i, name in enumerate(names):
        mask = ci == i
        sm = mask & strong
        x = np.clip((uv[sm, 0] / width * 8).astype(int), 0, 7)
        y = np.clip((uv[sm, 1] / height * 6).astype(int), 0, 5)
        np.add.at(grids[i], (y, x), 1)
        row = dict(
            image_id=int(ids[i]),
            filename=name,
            landmarks=int(mask.sum()),
            strong_landmarks=int(sm.sum()),
            strong_occupied_cells=int(np.count_nonzero(grids[i])),
            grid_cells=48,
            ...
            prior_pose_final_points_rmse_px=stats(before[mask]).get(
                "rmse", float("nan")
            ),
            translation_change_m=float(shift[i]),
            rotation_change_deg=float(angle[i]),
        )
        row["weak_support"] = (
            row["strong_landmarks"] < 50 or row["strong_occupied_cells"] < 16
        )
        rows.append(row)
        im = Image.open(
            (args.image_directory or args.dataset / "images") / name
        ).convert("RGB")
        draw = ImageDraw.Draw(im)
        for gx in range(1, 8):
            draw.line(
                (gx * width / 8, 0, gx * width / 8, height),
                fill=(150, 150, 150),
                width=1,
            )
        for gy in range(1, 6):
            draw.line(
                (0, gy * height / 6, width, gy * height / 6),
                fill=(150, 150, 150),
                width=1,
            )
        ...
        draw.text(
            (5, 5),
            f'Image {ids[i]} | strong {sm.sum()} / all {mask.sum()} | strong cells {row["strong_occupied_cells"]}/48 | green=strong orange=other',
            fill="white",
        )
    ...
    with (report / "per_frame_grid.csv").open("w") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["image_id"] + [f"row{r}_col{c}" for r in range(6) for c in range(8)]
        )
    ...
    def epipolar(poses, rot):
        ...
        fy = np.einsum("nji,nj->ni", f, y)
        denom = np.sum(fx[:, :2] ** 2 + fy[:, :2] ** 2, axis=1)
        return np.abs(np.sum(y * fx, axis=1)) / np.sqrt(np.maximum(denom, 1e-30))

    hp = epipolar(prior, rp)
    hf = epipolar(final, rf)
    summary = dict(
        pipeline=metrics,
        prior_pose_final_structure_reprojection=stats(before),
        final_reprojection=stats(after),
        heldout_prior_sampson_px=stats(hp),
        heldout_final_sampson_px=stats(hf),
        translation_change_m=stats(shift),
        rotation_change_deg=stats(angle),
    ...
    axes[0].plot(ids, [r["landmarks"] for r in rows], label="All landmarks")
    axes[0].plot(ids, [r["strong_landmarks"] for r in rows], label="Strong landmarks")
    axes[0].legend()
    axes[0].set_ylabel("Count")
    axes[1].plot(ids, [r["strong_coverage_percent"] for r in rows])
    axes[1].axhline(100 * 16 / 48, color="orange", linestyle="--")
    axes[1].set_ylabel("Strong grid coverage (%)")
    axes[2].plot(ids, [r["final_rmse_px"] for r in rows])
    axes[2].set_ylabel("Final RMSE (px)")
    axes[3].plot(ids, shift, label="Translation (m)")
    axes[3].plot(ids, angle, label="Rotation (deg)")
    axes[3].legend()
    axes[3].set_xlabel("Original image ID")
    for ax in axes:
        ax.grid(alpha=0.2)
    fig.tight_layout()
    fig.savefig(report / "per_frame_quality.png", dpi=150)
    plt.close(fig)
    fig, ax = plt.subplots(figsize=(16, 7))
    mesh = ax.imshow(
        np.log1p(grids.reshape(n, 48).T),
        aspect="auto",
        origin="lower",
        extent=[ids[0] - 0.5, ids[-1] + 0.5, -0.5, 47.5],
    )
    ax.set_xlabel("Original image ID")
    ax.set_ylabel("Grid cell: row × 8 + column")
    fig.colorbar(mesh, label="log(1 + strong landmark count)")
    fig.tight_layout()
    fig.savefig(report / "spatial_distribution.png", dpi=150)
    plt.close(fig)
    fig, axes = plt.subplots(1, 2, figsize=(13, 4))
    for values, label in [(hp, "LiDAR prior"), (hf, "Refined camera poses")]:
        ordered = np.sort(values[np.isfinite(values)])
        axes[0].plot(
            ordered, np.arange(1, len(ordered) + 1) / len(ordered), label=label
        )
    axes[0].set_xscale("symlog", linthresh=0.1)
    axes[0].set_xlabel("Held-out square-root Sampson error (px)")
    axes[0].set_ylabel("Cumulative fraction")
    axes[0].legend()
    axes[1].hist(after[np.isfinite(after)], bins=60, color="steelblue")
    axes[1].set_xlabel("Final reprojection error (px)")
    axes[1].set_ylabel("Retained observations")
    for ax in axes:
        ax.grid(alpha=0.2)
    fig.tight_layout()
    fig.savefig(report / "ba_performance.png", dpi=150)
    plt.close(fig)
    weakest = sorted(
        rows, key=lambda r: (r["strong_occupied_cells"], r["strong_landmarks"])
    )[:20]
...
"""
    for r in weakest:
        text += f"| [{r['image_id']}](frames/{r['image_id']:010d}.jpg) | {r['landmarks']} | {r['strong_landmarks']} | {r['strong_occupied_cells']} | {r['final_rmse_px']:.3f} |\n"
    text += "\n## Every selected frame\n\nGreen points are strong observations; orange points are other retained landmarks.\n\n"
    for r in rows:
        text += f"- [Image {r['image_id']}](frames/{r['image_id']:010d}.jpg): {r['strong_landmarks']} strong landmarks, {r['strong_occupied_cells']}/48 cells.\n"
