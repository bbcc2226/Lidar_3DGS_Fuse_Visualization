...
def camera_from_imu(sequence: Path, camera: str) -> np.ndarray:
    ...
    return baseline @ rect @ rigid(velo_cam["R"], velo_cam["T"]) @ rigid(imu_velo["R"], imu_velo["T"])
...
def oxts_camera_poses(sequence: Path, camera: str, ids: range) -> tuple[np.ndarray, R]:
    ...
    for row in rows:
        ...
        world_from_imu = rigid(R.from_euler("xyz", [roll, pitch, yaw]).as_matrix(), [x, y, alt])
...
def evaluate(centers: np.ndarray, rotations: R, gt_centers: np.ndarray, gt_rotations: R) -> dict:
...
def main() -> int:
    ...
    parser.add_argument("--sequence", type=Path, required=True, help="KITTI *_sync sequence directory")
    parser.add_argument("--camera", default="image_03")
    parser.add_argument("--start-id", type=int, required=True, help="image ID of the first pose")
    parser.add_argument("--poses", type=Path, nargs="+", required=True, help="TUM pose files (name=path or path)")
    ...
    for item in args.poses:
        text = str(item)
        name, path = text.split("=", 1) if "=" in text else (Path(text).parent.name, text)
    ...
    n = count.pop()
    gt_centers, gt_rotations = oxts_camera_poses(args.sequence, args.camera, range(args.start_id, args.start_id + n))

    report = {"frames": n, "image_id_range": [args.start_id, args.start_id + n - 1],
              "oxts_position_jitter_2nd_diff_m": stats(np.linalg.norm(np.diff(gt_centers, 2, axis=0), axis=1)),
              "trajectories": {}}
    for name, (centers, rotations) in trajectories.items():
        report["trajectories"][name] = evaluate(centers, rotations, gt_centers, gt_rotations)
    ...
    for name, m in report["trajectories"].items():
        a, r, t, rr, j = (m["ate_m"], m["rotation_vs_gt_deg"], m["rpe_1frame_translation_m"],
                          m["rpe_1frame_rotation_deg"], m["position_jitter_2nd_diff_m"])
        print(f"{name:24s} {a['median']:6.3f}/{a['p90']:6.3f}/{a['max']:6.3f}   {r['median']:10.2f} "
