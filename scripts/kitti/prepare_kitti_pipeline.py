...
def _required(
    values: dict[str, list[float]], key: str, count: int, path: Path
) -> list[float]:
...
def make_rectified_camera_calibration(
    sequence: Path, camera: str
    ...
    baseline_translation = _matvec(
        _inverse3(intrinsics), [row[3] for row in projection]
    )
    ...
    translation_camera_lidar = [
        rotated_translation[k] + baseline_translation[k] for k in range(3)
    ]
    extrinsic = [
        rotation_camera_lidar[row] + [translation_camera_lidar[row]] for row in range(3)
    ]
...
def read_image_records(sequence: Path, camera: str) -> list[tuple[int, str, float]]:
    ...
    timestamps = [
        _timestamp_seconds(line, timestamp_file, n)
    ...
    if len(ids) != len(timestamps):
        raise ValueError(
            f"{camera}: {len(ids)} PNGs but {len(timestamps)} timestamp records"
        )
    return [
        (image_id, images[image_id].name, timestamps[index])
...
def _validate_pose(pose: Any, path: Path, index: int) -> None:
    try:
        ...
        raise ValueError(
            f"{path}: record {index} has an invalid optimized pose"
        ) from exc
    if (
        len(translation) != 3
        or len(quaternion) != 4
        ...
        raise ValueError(
            f"{path}: record {index} pose must have finite translation[3] and quaternion[4]"
        )
...
def read_lio_records(
    lidar_dir: Path, pose_field: str, duplicate_policy: str = "last_revision"
    ...
    for index, record in enumerate(records):
        try:
            ...
            raise ValueError(
                f"{path}: record {index} has no numeric timestamp"
            ) from exc
        ...
        if cloud_ref:
            ...
            if not cloud.is_file():
                raise ValueError(
                    f"{path}: record {index} point cloud is missing: {cloud}"
                )
            record["saved_frame_path"] = str(cloud.resolve())
        elif record.get("key_frame_id") is not None:
            raise ValueError(
                f"{path}: record {index} has no saved_frame_path for its LiDAR cloud"
            )
    ...
    while index < len(records):
        ...
        while (
            ...
            and records[end]["timestamp"] == records[index]["timestamp"]
        ):
            end += 1
...
def prepare(
    config_path: Path, start_id: int | None = None, end_id: int | None = None
    ...
    missing_ids = [
        image_id for image_id in range(begin, end + 1) if image_id not in available_ids
    ]
    if missing_ids:
        raise ValueError(
            f"requested image IDs are missing (first missing ID: {missing_ids[0]})"
        )
    ...
    if clipped and outside_policy == "error":
        raise ValueError(
            f"{clipped} requested image(s) fall outside LIO pose time coverage"
        )
    if not valid:
        raise ValueError(
            "no requested image timestamps overlap the LIO pose trajectory"
        )
    ...
    for _, filename, _ in valid[1:]:
        ...
        if other_size != (width, height):
            raise ValueError(
                f"image dimensions vary inside range: {filename} is {other_size}, expected {(width, height)}"
            )
    ...
    with association_path.open("w", newline="") as stream:
        ...
        writer.writerow(
            [
                ...
                "pose_key_frame_id",
                "pose_timestamp",
                "pose_minus_image_plus_offset_seconds",
                "lidar_cloud",
            ]
        )
        for image_id, filename, image_time in valid:
            ...
            nearest_index = min(
                range(len(pose_times)), key=lambda j: abs(pose_times[j] - query)
            )
    ...
    for key, value in replacements.items():
        ...
        template, count = re.subn(
            pattern,
            lambda match: f"{match.group(1)}{key}: {rendered}",
            template,
            count=1,
        )
        if count != 1:
            raise ValueError(
                f"pipeline template must contain a top-level {key} setting"
            )
    ...
    summary = {
        ...
        "trajectory_time_range": [
            selected_poses[0]["timestamp"],
            selected_poses[-1]["timestamp"],
        ],
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "kitti_prepare.json",
        help="JSON configuration containing KITTI paths and inclusive image ID range",
    )
    parser.add_argument(
        "--start-image-id", type=int, help="override configured inclusive start ID"
    )
    parser.add_argument(
        "--end-image-id", type=int, help="override configured inclusive end ID"
    )
    parser.add_argument(
        "--prepare-only",
        action="store_true",
        help="write inputs/config but do not run C++",
    )
    parser.add_argument(
        "--cpp-executable", type=Path, help="override the root build executable path"
    )
    ...
    try:
        ...
        if args.prepare_only:
            print(
                "Prepared only. Run: build/lio_visual_ba_pipeline --config "
                + str(pipeline_config.resolve())
            )
            return 0
        config = json.loads(config_path.read_text())
        executable = args.cpp_executable or Path(
            config.get("cpp_executable", "build/lio_visual_ba_pipeline")
        )
        if not executable.is_absolute():
            executable = ROOT / executable
        if not executable.is_file():
            raise ValueError(
                f"C++ pipeline executable not found: {executable}; build with cmake --build build -j2"
            )
        return subprocess.run(
            [str(executable), "--config", str(pipeline_config.resolve())],
            cwd=ROOT,
            check=False,
        ).returncode
    except (
