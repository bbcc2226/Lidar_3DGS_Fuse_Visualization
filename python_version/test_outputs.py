...
p=argparse.ArgumentParser();p.add_argument('--data',type=Path,default=Path('data'));p.add_argument('--output',type=Path,default=Path('output/lio_camera_pose'));p.add_argument('--expected-images',type=int,default=-1);a=p.parse_args()
m=json.load(open(a.output/'evaluation.json'))
poses=[x.split() for x in open(a.output/'poses_optimized_tum.txt') if x.strip()]
available=sum(1 for line in open(a.data/'timestamps.txt') if (a.data/'undistorted'/line.split()[0]).exists())
...
assert m['lio_translation_delta_median_m']<=.3 and m['lio_rotation_delta_median_deg']<=3
timestamps=[]
for z in poses:
    vals=list(map(float,z));assert len(vals)==8 and all(math.isfinite(x) for x in vals)
...
for required in ['sparse/0/cameras.txt','sparse/0/images.txt','sparse/0/points3D.txt']:
    assert (a.output/required).stat().st_size>0,required
print(json.dumps({'pass':True,'poses':len(poses),'landmarks':n,'metrics':m},indent=2))
